...
export function getRandomCpuDecision(state: GameState, runtime: CpuRuntime, settings: CpuTeamSettings): CpuDecision | undefined {
  if (state.phase === "production") {
    ...
    if (teamId) {
      ...
      for (const candidate of getProductionCandidates(state, teamId)) byBase.set(candidate.baseId, [...(byBase.get(candidate.baseId) ?? []), candidate]);
      ...
      return getRandomCpuDecision(state, runtime, settings);
    }
  ...
  if (state.phase === "movement_input") {
    ...
    const movement = getTeamMovementCandidates(state, teamId);
    const teleports = getTeamTeleportCandidates(state, teamId);
    const actors = [
  ...
  if (state.phase === "attack_input") {
    ...
    if (teamId) {
      const attacker = getTeamAttackCandidates(state, teamId).find((entry) => !runtime.processedKeys.includes(`attack:${teamId}:${entry.attackerUnitId}`));
  ...
  if (state.phase === "reward_placement") {
    const candidate = state.teams
      ...
      .flatMap((team) => getRewardPlacementCandidates(state, team.id).map((placement) => ({ teamId: team.id, ...placement })))
  ...
  if (state.phase === "strategist_action_input") {
    ...
    const candidates = getStrategistActionCandidates(state, teamId);
