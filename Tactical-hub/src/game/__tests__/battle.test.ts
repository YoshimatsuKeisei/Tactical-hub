...
function targetDenominator(state: GameState, attackerUnitId: string, targetUnitId: string) {
  return getAttackCandidates(state, attackerUnitId).find((target) => target.unitId === targetUnitId)?.finalSuccessDenominator;
}
...
function saveAttacks(state: GameState, attacks: Array<[string, string, string]>) {
  return attacks.reduce((next, [teamId, attackerUnitId, targetUnitId]) => saveAttack(next, teamId, attackerUnitId, targetUnitId), state);
}
...
describe("battle", () => {
  ...
  it("does not mutate GameState when saving AttackIntent", () => {
    ...
    const planned = saveAttack(state, "team-1", "home-1-king", "enemy-adjacent");
  ...
  it("reduces target HP by 1 on success and removes units at 0 HP", () => {
    ...
    putUnit(state, "home-1-king", { kind: "tile", x: 4, y: 1 });
    addUnit(state, "enemy-adjacent", "team-2", "infantry", { kind: "tile", x: 5, y: 1 });
    const resolved = resolveBattle(saveAttack(state, "team-1", "home-1-king", "enemy-adjacent"), () => 0);
  ...
  it("keeps kings at 3 initial HP and defeats their team at 0 HP", () => {
    ...
    const resolved = resolveBattle(saveAttack(state, "team-1", "home-1-king", "home-2-king"), () => 0);
  ...
  it("resolves mutual attacks fully simultaneously", () => {
    const state = createInitialGameState();
    addUnit(state, "team-1-infantry-test", "team-1", "infantry", { kind: "tile", x: 4, y: 1 });
    addUnit(state, "team-2-infantry-test", "team-2", "infantry", { kind: "tile", x: 5, y: 1 });
    ...
    expect(resolved.units.find((unit) => unit.id === "team-1-infantry-test")?.position.kind).toBe("removed");
    expect(resolved.units.find((unit) => unit.id === "team-2-infantry-test")?.position.kind).toBe("removed");
  });
  ...
  it("resolves 3-unit cyclic attacks even when every attacker is defeated", () => {
    ...
    addUnit(state, "team-1-infantry-test", "team-1", "infantry", { kind: "tile", x: 4, y: 1 });
    addUnit(state, "team-2-infantry-test", "team-2", "infantry", { kind: "tile", x: 5, y: 1 });
    addUnit(state, "team-3-infantry-test", "team-3", "infantry", { kind: "tile", x: 5, y: 2 });
    ...
    expect(resolved.units.find((unit) => unit.id === "team-1-infantry-test")?.position.kind).toBe("removed");
    expect(resolved.units.find((unit) => unit.id === "team-2-infantry-test")?.position.kind).toBe("removed");
    expect(resolved.units.find((unit) => unit.id === "team-3-infantry-test")?.position.kind).toBe("removed");
  });
  ...
  it("keeps an attack if the attacker is defeated in the same battle", () => {
    ...
    expect(resolved.units.find((unit) => unit.id === "team-2-infantry-test")?.position.kind).toBe("removed");
    expect(resolved.units.find((unit) => unit.id === "home-1-king")?.hp).toBe(2);
  });

  it("combines multiple successful attacks into target damage", () => {
    const state = createInitialGameState();
    addUnit(state, "team-1-infantry-a", "team-1", "infantry", { kind: "tile", x: 4, y: 1 });
    addUnit(state, "team-1-infantry-b", "team-1", "infantry", { kind: "tile", x: 4, y: 2 });
    addUnit(state, "enemy-king", "team-2", "king", { kind: "tile", x: 5, y: 1 });
  ...
  it("does not change simultaneous battle results when attack intent order changes", () => {
    ...
    addUnit(first, "team-1-infantry-test", "team-1", "infantry", { kind: "tile", x: 4, y: 1 });
    addUnit(first, "team-2-infantry-test", "team-2", "infantry", { kind: "tile", x: 5, y: 1 });
    ...
    expect(firstResolved.units.map((unit) => [unit.id, unit.hp, unit.position.kind])).toEqual(
      secondResolved.units.map((unit) => [unit.id, unit.hp, unit.position.kind]),
    );
  });

  it("does not depend on unitId ordering for simultaneous outcomes", () => {
    const state = createInitialGameState();
    addUnit(state, "z-attacker", "team-1", "infantry", { kind: "tile", x: 4, y: 1 });
    addUnit(state, "a-defender", "team-2", "infantry", { kind: "tile", x: 5, y: 1 });
    ...
    expect(resolved.units.find((unit) => unit.id === "z-attacker")?.position.kind).toBe("removed");
    expect(resolved.units.find((unit) => unit.id === "a-defender")?.position.kind).toBe("removed");
  });
  ...
  it("uses the unit-type attack success table", () => {
    ...
    expect(getBaseAttackDenominator("strategist", "infantry", normal)).toBeNull();
  });

  it("limits engineers to siege attacks and allows kings inside bases", () => {
    const state = createInitialGameState();
    addUnit(state, "team-1-engineer-test", "team-1", "engineer", { kind: "tile", x: 8, y: 1 });
    addUnit(state, "enemy-ground-infantry", "team-2", "infantry", { kind: "tile", x: 9, y: 1 });
    addUnit(state, "enemy-base-king", "team-2", "king", { kind: "base", baseId: "neutral-north", slotId: "slot_1_1" });
    ...
    expect(getBaseAttackDenominator("engineer", "infantry", { targetInBase: true })).toBe(5);
    expect(getBaseAttackDenominator("engineer", "engineer", { targetInBase: true })).toBe(6);
    expect(getBaseAttackDenominator("engineer", "king", { targetInBase: true })).toBe(5);
    expect(getBaseAttackDenominator("engineer", "infantry", { targetInBase: false })).toBeNull();
  });

  it("applies water ninja combat restrictions", () => {
    const state = createInitialGameState();
    addUnit(state, "team-1-infantry-test", "team-1", "infantry", { kind: "tile", x: 4, y: 1 });
    addUnit(state, "team-1-ground-ninja", "team-1", "ninja", { kind: "tile", x: 4, y: 3 });
    addUnit(state, "team-1-water-ninja", "team-1", "ninja", { kind: "water", x: 4, y: 2 });
    addUnit(state, "team-2-water-ninja", "team-2", "ninja", { kind: "water", x: 5, y: 2 });
    addUnit(state, "team-2-ground-infantry", "team-2", "infantry", { kind: "tile", x: 5, y: 3 });

    expect(targetIds(state, "team-1-infantry-test")).not.toContain("team-2-water-ninja");
    expect(targetIds(state, "team-1-water-ninja")).not.toContain("team-2-ground-infantry");
    expect(targetIds(state, "team-1-water-ninja")).toContain("team-2-water-ninja");
    expect(targetIds(state, "team-1-ground-ninja")).not.toContain("team-2-water-ninja");
  });
  ...
  it("supports encouragement as a non-stacking denominator reduction", () => {
    ...
    expect(getFinalAttackDenominator("infantry", "archer", { targetInBase: false, encouraged: true })).toBe(4);
    expect(applyEncouragementToDenominator(applyEncouragementToDenominator(5, true), false)).toBe(4);
  });

  it("does not encourage water attackers even if they carry an encouraged status", () => {
    const state = createInitialGameState();
    const attacker = addUnit(state, "team-1-water-ninja", "team-1", "ninja", { kind: "water", x: 4, y: 2 });
    attacker.statuses.push({ kind: "encouraged" }, { kind: "encouraged" });
    addUnit(state, "team-2-water-ninja", "team-2", "ninja", { kind: "water", x: 5, y: 2 });

    expect(targetDenominator(state, "team-1-water-ninja", "team-2-water-ninja")).toBe(6);
  });

  it("keeps battle-start encouragement even if the strategist is defeated in that battle", () => {
    const state = createInitialGameState();
    const attacker = addUnit(state, "team-1-infantry-test", "team-1", "infantry", { kind: "tile", x: 4, y: 1 });
    attacker.statuses.push({ kind: "encouraged" });
    putUnit(state, "home-1-strategist", { kind: "tile", x: 5, y: 1 });
    addUnit(state, "team-2-archer-test", "team-2", "archer", { kind: "tile", x: 4, y: 2 });
    addUnit(state, "team-2-infantry-test", "team-2", "infantry", { kind: "tile", x: 6, y: 1 });
    ...
    expect(resolved.units.find((unit) => unit.id === "home-1-strategist")?.position.kind).toBe("removed");
    expect(resolved.logs.some((log) => log.message.includes("team-1-infantry-test") && log.message.includes("final 1/4"))).toBe(true);
  });

  it("sorts attack candidates by provisional deterministic success priority", () => {
    const state = createInitialGameState();
    addUnit(state, "team-1-infantry-test", "team-1", "infantry", { kind: "tile", x: 4, y: 1 });
    addUnit(state, "enemy-normal", "team-2", "infantry", { kind: "tile", x: 5, y: 1 });
    addUnit(state, "enemy-favorable", "team-2", "archer", { kind: "tile", x: 4, y: 2 });
    addUnit(state, "enemy-king", "team-2", "king", { kind: "tile", x: 5, y: 2 });

    const ordered = targetIds(state, "team-1-infantry-test");
    expect(ordered.slice(0, 3)).toEqual(["enemy-king", "enemy-favorable", "enemy-normal"]);
  ...
  it("sorts wounded enemies before full-HP enemies under otherwise equal conditions", () => {
    const state = createInitialGameState();
    addUnit(state, "team-1-archer-test", "team-1", "archer", { kind: "tile", x: 4, y: 1 });
    const wounded = addUnit(state, "enemy-king-wounded", "team-2", "king", { kind: "tile", x: 5, y: 1 });
    addUnit(state, "enemy-king-full", "team-2", "king", { kind: "tile", x: 6, y: 1 });
    wounded.hp = 2;

    expect(targetIds(state, "team-1-archer-test").slice(0, 2)).toEqual(["enemy-king-wounded", "enemy-king-full"]);
  });
});
